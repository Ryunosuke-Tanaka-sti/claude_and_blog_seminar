...
## 追加改善（2025-11-18 v1.1）

### パスフィルターの追加
**問題**: `docs/` ディレクトリ（計画・仕様書）の変更時にも不要なデプロイが実行されていた

**解決策**: ワークフローのトリガーにパスフィルターを追加
```yaml
on:
  push:
    branches:
...
**効果**:
- ✅ `docs/` の変更時にデプロイをスキップ
- ✅ GitHub Actions使用量の削減
- ✅ CI/CDキューの効率化
- ✅ 3フェーズ開発（計画→実装→検証）との相性が向上

**3フェーズ開発との関連**:
- 計画フェーズ（`docs/` での作業）: デプロイなし
- 実装フェーズ（`application/` での作業）: デプロイ実行
- 検証フェーズ（`docs/research/` での作業）: デプロイなし
