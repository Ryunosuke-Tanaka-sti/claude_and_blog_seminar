...
```
計画フェーズ → 実装フェーズ → 検証フェーズ → (確定仕様化)
   (仕様書)      (コード)      (差分分析)     (運用開始時)
```

※ 検証後、運用に乗せる際は確定仕様として `docs/spec/` に保管します。
...
│       ├── spec-confirmed-template.md  # 確定仕様テンプレート
...
4. **確定仕様への昇格**（運用開始時）:
   ...
   # 3. 確定仕様として保存
   # この仕様は運用中の安定版として管理され、
   # 他のドキュメントや新機能開発時の参照元として使用される
   ```

### 4. 確定仕様の管理（`docs/spec/`配下で保管）

**目的**: 実装・検証が完了した機能の安定版仕様を保管・管理する

#### 確定仕様のワークフロー

```
features/[feature-name]/spec.md（計画時の仕様）
    ↓ 実装フェーズ
application/[workspace]/（実装コード）
    ↓ 検証フェーズ
docs/research/[feature-name]-review.md（検証結果）
    ↓ 運用開始時に昇格
spec/[category]/[feature-name].md（確定仕様）
```

#### 確定仕様の役割

- **参照の安定性**: 運用中の機能の仕様を安定的に参照できる
- **新機能開発の参照元**: 関連機能開発時の仕様参照元として使用
- **ドキュメント管理**: 計画中（features/）と確定版（spec/）を分離管理
